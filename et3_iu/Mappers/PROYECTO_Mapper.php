...
public function buscarMiembro(Miembro $miembro)
{
  ...
  public function buscarId($proyectoId){
    ...
    public function consultarMiembros($idProyecto) {
        $this ->conectarBD();
        $sql = "SELECT EMPLEADOS.EMP_USER,EMPLEADOS.EMP_PASSWORD,EMPLEADOS.EMP_NOMBRE,EMPLEADOS.EMP_APELLIDO,EMPLEADOS.EMP_EMAIL,EMPLEADOS.EMP_TIPO,EMPLEADOS.EMP_ESTADO FROM EMPLEADOS,PROYECTO_MIEMBRO WHERE PROYECTO_MIEMBRO.EMP_USER NOT IN (SELECT EMP_USER FROM PROYECTO_MIEMBRO WHERE ID_PROYECTO =$idProyecto) AND PROYECTO_MIEMBRO.EMP_USER = EMPLEADOS.EMP_USER ";
